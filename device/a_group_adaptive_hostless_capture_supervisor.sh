...
# Fixed hostless supervisor for exactly one same-session center-AF followed by
# one A1-A5 gain-1 capture using a strictly bounded measured exposure plan.
#
# This is deliberately not a general root bridge.  It accepts no arguments,
# reads one canonical five-value plan from the fixed app-private path, verifies
# the exact reviewed child and preload payloads, invokes only the adaptive
# inline-AF A-group path, and requests a normal reboot after every possible
...
APP_DIR=/data/data/io.github.tobiasbrummer.lightl16.adaptiveagroupcapture/files
...
APP_EXTERNAL_RESULT=/sdcard/Android/data/io.github.tobiasbrummer.lightl16.adaptiveagroupcapture/files/light-l16-adaptive-a-group-capture-last-display.txt
APP_ARM_VALUE=L16_HOSTLESS_A_GROUP_ADAPTIVE_INLINE_AF_CAPTURE_SUPERVISOR_ONCE_V1
PLAN_VERSION=L16_ADAPTIVE_A_GROUP_PLAN_V1

CHILD=/data/local/tmp/light_l16_a_group_adaptive_capture_once.sh
CHILD_RESULT=/data/local/tmp/light_l16_a_group_adaptive_capture.result
CHILD_ARM=/data/local/tmp/light_l16_a_group_adaptive_capture.armed
CHILD_ARM_VALUE=A_GROUP_ADAPTIVE_INLINE_AF_CAPTURE_GAIN_1.0_ONCE
AF_SHIM=/data/local/tmp/liblcc_a1_focus_capture_shim.so
EXPECTED_CHILD_SIZE=40130
EXPECTED_CHILD_SHA1=fd4b2e892daa002cf777fa3365683250451cedc2
...
finish() {
    ...
    /system/bin/sync
    # Android can be suspended before its polling thread mirrors the completed
    # private manifest.  The app has already created this exact external file;
    # copy the finished result best-effort so the diagnostic survives reboot.
    # Reporting must never change capture or reboot policy.
    if [ -f "$APP_EXTERNAL_RESULT" ]; then
        cp "$APP_RESULT" "$APP_EXTERNAL_RESULT" 2>/dev/null || true
        /system/bin/sync
    fi
...
printf 'supervisor=L16_HOSTLESS_A_GROUP_ADAPTIVE_INLINE_AF_CAPTURE_V1\n'
printf 'policy=single_bounded_measured_same_session_A1_A5_center_AF_gain_1_capture_and_reboot\n'
...
# Parse and then reconstruct the canonical line.  This rejects extra fields,
# whitespace variants, signs, shell syntax, and additional lines without ever
# evaluating app-controlled text as code.
[ -f "$APP_PLAN" ] || fail app_plan_missing
[ -r "$APP_PLAN" ] || fail app_plan_unreadable
APP_PLAN_SIZE=$(/system/bin/toybox wc -c < "$APP_PLAN") || \
    fail cannot_size_app_plan
valid_decimal "$APP_PLAN_SIZE" || fail invalid_app_plan_size
[ "$APP_PLAN_SIZE" -ge 1 ] && [ "$APP_PLAN_SIZE" -le 160 ] || \
    fail app_plan_size_out_of_range
APP_PLAN_LINES=$(/system/bin/toybox wc -l < "$APP_PLAN") || \
    fail cannot_count_app_plan_lines
[ "$APP_PLAN_LINES" = 1 ] || fail app_plan_must_be_one_line
IFS=' ' read -r APP_PLAN_VERSION EXPOSURE_A1 EXPOSURE_A2 EXPOSURE_A3 \
    EXPOSURE_A4 EXPOSURE_A5 EXTRA < "$APP_PLAN" || fail cannot_read_app_plan
[ -z "$EXTRA" ] || fail app_plan_has_extra_fields
[ "$APP_PLAN_VERSION" = "$PLAN_VERSION" ] || fail unexpected_app_plan_version
for EXPOSURE_VALUE in \
    "$EXPOSURE_A1" "$EXPOSURE_A2" "$EXPOSURE_A3" "$EXPOSURE_A4" "$EXPOSURE_A5"
do
    valid_decimal "$EXPOSURE_VALUE" || fail invalid_app_plan_exposure
    [ "$EXPOSURE_VALUE" -ge 10000 ] || fail app_plan_exposure_below_10000ns
    [ "$EXPOSURE_VALUE" -le 20000000 ] || fail app_plan_exposure_above_20000000ns
done
[ "$EXPOSURE_A2" -eq "$EXPOSURE_A1" ] || \
    fail app_plan_a2_must_equal_a1
[ "$EXPOSURE_A3" -ge "$EXPOSURE_A1" ] || fail app_plan_a3_below_a1
[ "$EXPOSURE_A4" -ge "$EXPOSURE_A3" ] || fail app_plan_a4_below_a3
[ "$EXPOSURE_A5" -ge "$EXPOSURE_A4" ] || fail app_plan_a5_below_a4
[ "$EXPOSURE_A5" -gt "$EXPOSURE_A1" ] || fail app_plan_hdr_ladder_collapsed
CANONICAL_PLAN="$PLAN_VERSION $EXPOSURE_A1 $EXPOSURE_A2 $EXPOSURE_A3 $EXPOSURE_A4 $EXPOSURE_A5"
APP_PLAN_TEXT=$(cat "$APP_PLAN") || fail cannot_load_app_plan
[ "$APP_PLAN_TEXT" = "$CANONICAL_PLAN" ] || fail app_plan_not_canonical
APP_PLAN_SHA1=$(/system/bin/toybox sha1sum "$APP_PLAN") || \
    fail cannot_hash_app_plan
APP_PLAN_SHA1=${APP_PLAN_SHA1%% *}
valid_sha1 "$APP_PLAN_SHA1" || fail invalid_app_plan_sha1
EXPECTED_EXPOSURE_PLAN="A1:$EXPOSURE_A1,A2:$EXPOSURE_A2,A3:$EXPOSURE_A3,A4:$EXPOSURE_A4,A5:$EXPOSURE_A5"
printf 'app_plan_size=%s\n' "$APP_PLAN_SIZE"
printf 'app_plan_sha1=%s\n' "$APP_PLAN_SHA1"
printf 'app_plan_exposure_order=%s\n' "$EXPECTED_EXPOSURE_ORDER"
printf 'app_plan_exposure_values_ns=%s,%s,%s,%s,%s\n' \
    "$EXPOSURE_A1" "$EXPOSURE_A2" "$EXPOSURE_A3" "$EXPOSURE_A4" "$EXPOSURE_A5"
printf 'app_plan_validation=PASS\n'
...
case "$WORKDIR" in
    /data/local/tmp/light_l16_a_group_adaptive_capture_run.*)
        WORK_PID=${WORKDIR#/data/local/tmp/light_l16_a_group_adaptive_capture_run.}
        case "$WORK_PID" in
...
SUPERVISOR_REASON=same_session_adaptive_a_group_focus_and_capture_recorded_and_child_cleanup_verified
