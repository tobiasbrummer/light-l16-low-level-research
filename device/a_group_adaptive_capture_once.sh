...
# DANGER: after all fixed preconditions pass, this executes one bounded lcc
# capture. It is intentionally not a general camera or root wrapper.  The only
# variable input is exactly five decimal exposure times supplied by the
# hash-pinned supervisor.  Both processes validate the values independently.
...
if [ "$DYNAMIC_ARGUMENT_COUNT" -eq 5 ]; then
    EXPOSURE_A1=$1
    EXPOSURE_A2=$2
    EXPOSURE_A3=$3
    EXPOSURE_A4=$4
...
EXPOSURE_ARGS="$EXPOSURE_A1 $EXPOSURE_A2 $EXPOSURE_A3 $EXPOSURE_A4 $EXPOSURE_A5"
EXPOSURE_PLAN="A1:$EXPOSURE_A1,A2:$EXPOSURE_A2,A3:$EXPOSURE_A3,A4:$EXPOSURE_A4,A5:$EXPOSURE_A5"

[ "$0" = /data/local/tmp/light_l16_a_group_adaptive_capture_once.sh ] || {
    printf 'refusing unexpected invocation path: %s\n' "$0" >&2
    exit 2
}

OUT=/data/local/tmp/light_l16_a_group_adaptive_capture.result
ARM_FILE=/data/local/tmp/light_l16_a_group_adaptive_capture.armed
ARM_VALUE=A_GROUP_ADAPTIVE_INLINE_AF_CAPTURE_GAIN_1.0_ONCE
WORK_PREFIX=/data/local/tmp/light_l16_a_group_adaptive_capture_run
MODE=A_GROUP_ADAPTIVE_INLINE_CENTER_AF_CAPTURE_ONCE
MASK0=3E
MASK1=00
MASK2=00
SELECTION_DESCRIPTION='mask=3E 00 00 modules=A1-A5 asics=1,2 adaptive_exposures=required inline_af_shim=required roi=center_50_percent'
...
# Validate the supervisor-provided plan before creating a work directory or
# touching any camera state.  Short exposures down to the Camera2-advertised
# order of magnitude are accepted; the upper bound remains the proven 20 ms.
[ "$DYNAMIC_ARGUMENT_COUNT" -eq 5 ] || \
    fail unexpected_dynamic_exposure_argument_count
for EXPOSURE_VALUE in $EXPOSURE_ARGS; do
    case "$EXPOSURE_VALUE" in
        ""|*[!0-9]*) fail invalid_dynamic_exposure_value ;;
    esac
    [ "$EXPOSURE_VALUE" -ge 10000 ] || fail dynamic_exposure_below_10000ns
    [ "$EXPOSURE_VALUE" -le 20000000 ] || fail dynamic_exposure_above_20000000ns
done
[ "$EXPOSURE_A2" -eq "$EXPOSURE_A1" ] || \
    fail provisional_a2_must_equal_highlight_safe_a1
[ "$EXPOSURE_A3" -ge "$EXPOSURE_A1" ] || fail a3_below_a1
[ "$EXPOSURE_A4" -ge "$EXPOSURE_A3" ] || fail a4_below_a3
[ "$EXPOSURE_A5" -ge "$EXPOSURE_A4" ] || fail a5_below_a4
[ "$EXPOSURE_A5" -gt "$EXPOSURE_A1" ] || fail adaptive_hdr_ladder_collapsed
printf 'adaptive_plan_validation=PASS\n'
...
if [ "$USE_ASYNC_SHIM" = "yes" ]; then
    ...
    if [ "$MODE" = "A_GROUP_ADAPTIVE_INLINE_CENTER_AF_CAPTURE_ONCE" ]; then
