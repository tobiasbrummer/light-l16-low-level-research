...
# one A1-A5 20 ms / gain 1 capture.
#
...
# fixed inline-AF A-group path, and requests a normal reboot after every possible
...
APP_DIR=/data/data/io.github.tobiasbrummer.lightl16.agroupcapture/files
SUPERVISOR=$APP_DIR/s.sh
APP_CHILD=$APP_DIR/c.sh
APP_AF_SHIM=$APP_DIR/f.so
APP_RESULT=$APP_DIR/r.txt
APP_ARM=$APP_DIR/a
APP_ARM_VALUE=L16_HOSTLESS_A_GROUP_INLINE_AF_CAPTURE_SUPERVISOR_ONCE_V1

CHILD=/data/local/tmp/light_l16_a_group_inline_af_capture_once.sh
CHILD_RESULT=/data/local/tmp/light_l16_a_group_inline_af_capture.result
CHILD_ARM=/data/local/tmp/light_l16_a_group_inline_af_capture.armed
CHILD_ARM_VALUE=A_GROUP_INLINE_AF_CAPTURE_20000000NS_GAIN_1.0_ONCE
AF_SHIM=/data/local/tmp/liblcc_a1_focus_capture_shim.so
EXPECTED_CHILD_SIZE=54223
EXPECTED_CHILD_SHA1=abdf88fce0025a2a1994198228373d0825c73ec5
...
printf 'supervisor=L16_HOSTLESS_A_GROUP_INLINE_AF_CAPTURE_V1\n'
printf 'policy=single_fixed_same_session_A1_A5_center_AF_then_20ms_gain_1_capture_and_reboot\n'
...
case "$WORKDIR" in
    /data/local/tmp/light_l16_a_group_inline_af_capture_run.*)
        WORK_PID=${WORKDIR#/data/local/tmp/light_l16_a_group_inline_af_capture_run.}
        case "$WORK_PID" in
...
SUPERVISOR_REASON=same_session_a_group_focus_and_capture_recorded_and_child_cleanup_verified
