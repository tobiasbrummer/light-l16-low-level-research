...
/**
 * Owner-controlled, one-install/one-shot A1 capture for the Light L16.
 *
...
public final class MainActivity extends Activity {
    ...
    private static final String EXPECTED_SUPERVISOR_SHA256 =
        "bcc9b47a873fd75a252f5282bdf7472d45cdb6d05269bcb463cfaf515e3dbe99";
    private static final long EXPECTED_CHILD_SIZE = 22189L;
    private static final String EXPECTED_CHILD_SHA256 =
        "221e9c2b9d58dfb4870f1b8c4c07102f2e10899ab7b86662c567dea158ceae44";
    ...
    private DarknessCheck darknessCheck;
    private boolean running;
    private boolean uiArmed;
    private boolean preflightPassed;
    private boolean darknessConfirmed;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        darknessButton = new Button(this);
        darknessButton.setText("2. DUNKELHEIT PRÜFEN");
    ...
    private void showPersistentState() {
        try {
            ...
            if (result.exists() && result.length() > 0L) {
                ...
                } else if (validPartialResult(resultText)) {
                    report.append("app_interpretation=PARTIAL_MANIFEST_REBOOT_REQUESTED\n");
                    report.append("captures_completed=")
                        .append(field(resultText, "captures_completed")).append('\n');
    ...
    private void startPreflight() {
                ...
                runOnUiThread(new Runnable() {
                    ...
                    public void run() {
                        ...
                        if (result.passed) {
                            ...
                            darknessButton.setEnabled(true);
                            captureButton.setEnabled(darknessConfirmed);
        ...
        }, "l16-dark-frame-read-only-preflight").start();
    }

    private void startDarknessCheck() {
        if (running || !preflightPassed || privateFile(SPENT_NAME).exists()) {
            return;
        }
        ...
        darknessButton.setEnabled(false);
        setOutputText(
            "Camera2 misst bei maximaler Empfindlichkeit, ob das Objektiv "
                + "wirklich abgedeckt ist. Die Kamera wird danach wieder "
                + "geschlossen …\n"
        );
        darknessCheck = new DarknessCheck();
        darknessCheck.start(this, new DarknessCheck.Callback() {
            @Override
            public void onResult(final boolean dark, final String report) {
                runOnUiThread(new Runnable() {
                    @Override public void run() {
                        running = false;
                        onDarknessCheckResult(dark, report);
                    }
                });
            }
        });
    ...
    public void onRequestPermissionsResult(int requestCode, String[] permissions,
        ...
        if (grantResults.length > 0
            ...
            setOutputText("camera_permission=DENIED\n"
                + "action=grant_the_camera_permission_for_the_darkness_check\n");
        }
    }
    ...
    void onDarknessCheckResult(boolean dark, String report) {
        ...
        if (dark) {
            text.append("darkness=CONFIRMED\n");
            darknessButton.setText("2. DUNKELHEIT BESTÄTIGT");
        } else {
            text.append("darkness=REFUSED\n");
            text.append("action=cover_the_lens_completely_and_measure_again\n");
            darknessButton.setEnabled(true);
            darknessButton.setText("2. DUNKELHEIT ERNEUT PRÜFEN");
        }
        setOutputText(text.toString());
        captureButton.setEnabled(preflightPassed && darknessConfirmed && uiArmed);
    }
    ...
    private void startCapture() {
        ...
        if (!preflightPassed) {
            setOutputText("refusing capture without preflight\n");
            return;
        }
        if (!darknessConfirmed) {
            setOutputText("refusing capture without darkness check\n");
            return;
        }
        if (!uiArmed) {
            return;
        ...
        }, "l16-dark-frame-series-once").start();
    }
    ...
    private String runCaptureOnce() {
        ...
        try {
            ...
            File asyncShim = privateFile("n.so");
            File result = privateFile("r.txt");
            File arm = privateFile("a");
            File spent = privateFile(SPENT_NAME);
            if (supervisor.exists() || child.exists() || asyncShim.exists()
                    || result.exists()
            ...
            copyAssetToPrivateFile(ASYNC_SHIM_ASSET, asyncShim);
            ...
            verifyFile(asyncShim, EXPECTED_ASYNC_SHIM_SIZE,
                EXPECTED_ASYNC_SHIM_SHA256, "staged_async_shim");
            ...
            // Camera2 must be gone before root touches the camera path.  The
            // child independently re-checks this through dumpsys before it
            // runs lcc, so this is the app-side half of a two-sided check.
            if (darknessCheck != null) {
                darknessCheck.close();
                darknessCheck = null;
            }
            if (!DarknessCheck.isClosed()) {
                throw new IllegalStateException("camera2_still_open_before_trigger");
            }
            report.append("camera2_closed_before_trigger=yes\n");
            ...
            if (!completed) {
                ...
                report.append("captures_completed=")
                    .append(field(resultText, "captures_completed")).append('\n');
                report.append("artifact=24_new_LRI_retained_in_DCIM_camera\n");
            } else if (validPartialResult(resultText)) {
                report.append("result=PARTIAL_REBOOT_EXPECTED\n");
                report.append("captures_completed=")
                    .append(field(resultText, "captures_completed")).append('\n');
                report.append("series_aborted_at=")
                    .append(field(resultText, "series_aborted_at")).append('\n');
                report.append("series_abort_reason=")
                    .append(field(resultText, "series_abort_reason")).append('\n');
                report.append("artifact=listed_LRIs_retained_in_DCIM_camera\n");
            } else if (validPreflightFailure(resultText)) {
            ...
            if (!triggerAttempted) {
                ...
                deleteQuietly(privateFile("n.so"));
    ...
    private void inspectTargetAndAssets(StringBuilder report, boolean requireNoStaging)
        ...
        AssetInfo asyncShimAsset = inspectAsset(ASYNC_SHIM_ASSET);
        ...
        report.append("async_shim_asset_size=").append(asyncShimAsset.size).append('\n');
        report.append("async_shim_asset_sha256=")
            .append(asyncShimAsset.sha256).append('\n');
        ...
        if (asyncShimAsset.size != EXPECTED_ASYNC_SHIM_SIZE
                || !EXPECTED_ASYNC_SHIM_SHA256.equals(asyncShimAsset.sha256)) {
            throw new IllegalStateException("unexpected_async_shim_asset");
        }
    }
    ...
    private static boolean hasCompleteMarker(String text) {
        ...
        return "PASS".equals(value) || "PARTIAL".equals(value)
            || "FAIL".equals(value) || "PREFLIGHT_FAIL".equals(value);
    }

    private static boolean validPassResult(String text) {
        return validSeriesResult(text)
            && "PASS".equals(field(text, "supervisor_complete"))
            && "PASS".equals(field(text, "child_final_status"))
            && EXPECTED_CAPTURES.equals(field(text, "captures_completed"));
    }

    // A series that stopped early still returns frames that passed every
    // per-capture check, so it is reported as its own outcome rather than as
    // a failure.  The completed count is what makes it interpretable.
    private static boolean validPartialResult(String text) {
        return validSeriesResult(text)
            && "PARTIAL".equals(field(text, "supervisor_complete"))
            && validDecimalAtLeast(field(text, "captures_completed"), 1L);
    }
