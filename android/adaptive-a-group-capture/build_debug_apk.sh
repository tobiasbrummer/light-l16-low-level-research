...
OUTPUT_DIR="$PROJECT_ROOT/.build/adaptive-a-group-capture"
APK="$OUTPUT_DIR/light-l16-adaptive-a-group-hdr-capture-debug.apk"
SUPERVISOR="$PROJECT_ROOT/device/a_group_adaptive_hostless_capture_supervisor.sh"
CHILD="$PROJECT_ROOT/device/a_group_adaptive_capture_once.sh"
AF_SHIM_BUILDER="$PROJECT_ROOT/host/build_lcc_a1_focus_capture_shim.sh"
EXPECTED_SUPERVISOR_SIZE=17301
EXPECTED_SUPERVISOR_SHA256=0b1375f406b48987fe77f74b8b0c6612fe1bf5018794f26a8d9646397b92c9b9
EXPECTED_CHILD_SIZE=40130
EXPECTED_CHILD_SHA256=edb26ddd7894e5913204cadf2d2176802ef10de3e79018a37896182cd6256311
...
TEMP_DIR=$(mktemp -d "${TMPDIR:-/tmp}/light-l16-adaptive-a-group.XXXXXX")
trap 'rm -rf "$TEMP_DIR"' EXIT HUP INT TERM
mkdir -p "$TEMP_DIR/classes" "$TEMP_DIR/dex" "$TEMP_DIR/assets" "$OUTPUT_DIR"
cp "$SUPERVISOR" \
    "$TEMP_DIR/assets/a_group_adaptive_hostless_capture_supervisor.sh"
cp "$CHILD" "$TEMP_DIR/assets/a_group_adaptive_capture_once.sh"
