...
public final class CaptureActivity extends Activity {
    ...
    private static final String PRIVATE_DIR =
        "/data/data/io.github.tobiasbrummer.lightl16.adaptiveagroupcapture/files";
    private static final String USER_ZERO_PRIVATE_DIR =
        "/data/user/0/io.github.tobiasbrummer.lightl16.adaptiveagroupcapture/files";
    ...
    private static final String SUPERVISOR_ASSET =
        "a_group_adaptive_hostless_capture_supervisor.sh";
    private static final String CHILD_ASSET = "a_group_adaptive_capture_once.sh";
    private static final String AF_SHIM_ASSET =
        "liblcc_a1_focus_capture_shim.so";
    private static final long EXPECTED_SUPERVISOR_SIZE = 17301L;
    private static final String EXPECTED_SUPERVISOR_SHA256 =
        "9fc456975de0992618f304d2606fe3e2fbb74e60359785b559ad8825525633fb";
    private static final long EXPECTED_CHILD_SIZE = 40130L;
    private static final String EXPECTED_CHILD_SHA256 =
        "a3df7398e2c54f631a555e9b8754f5048e83ef3712379a23681ca4f8f0e71e47";
    ...
    private static final String ARM_VALUE =
        "L16_HOSTLESS_A_GROUP_ADAPTIVE_INLINE_AF_CAPTURE_SUPERVISOR_ONCE_V1";
    private static final String SPENT_VALUE =
        "L16_HOSTLESS_A_GROUP_ADAPTIVE_INLINE_AF_CAPTURE_SPENT_V1";
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        explanation.setText(
            "Adaptive Same-Session-Aufnahme: Der eingebettete, fest begrenzte "
                ...
                + "den unmittelbar zuvor gemessenen, unten protokollierten "
                + "Belichtungszeiten und Gain 1 ausgelöst, genau einmal pro "
                + "App-Installation.\n\n"
                + "Nach dem bewussten Aufnahmetastendruck im Messfenster prüft "
                + "diese Ansicht Gerät, Messplan, Root-Runner, Skripte und "
                + "Fokus-Hook zuerst nur lesend und startet bei PASS direkt "
                + "die echte Aufnahme. "
        ...
        preflightButton.setText("AUTOMATISCHE VORPRÜFUNG");
        ...
        captureButton.setText("A1-A5 MIT GEMESSENEM PLAN AUSLÖSEN");
        ...
        if (!privateFile(SPENT_NAME).exists()) {
            if (consumeUiLaunchArm()) {
                startPreflight();
            } else {
                preflightButton.setEnabled(false);
                captureButton.setEnabled(false);
                setOutputText(
                    "capture=REFUSED\nreason=missing_or_invalid_ui_launch_arm\n"
    ...
    private void startCapture() {
        ...
        captureButton.setEnabled(false);
        setOutputText(
            "Der adaptive Root-Ablauf startet lcc. Innerhalb derselben offenen "
                + "HAL-Sitzung wird Center-AF ausgeführt; nur bei "
                + "FOCUSED_LOCKED werden anschließend genau A1-A5 mit dem "
                + "gemessenen Plan ausgelöst …\n"
        );
        new Thread(new Runnable() {
    ...
    private void inspectTargetAndAssets(StringBuilder report, boolean requireNoStaging)
        ...
        long[] plan = readAndValidatePlan(privateFile(PLAN_NAME));
        report.append("plan_version=").append(PLAN_VERSION).append('\n');
        report.append("plan_exposure_order=A1,A2,A3,A4,A5\n");
        report.append("plan_exposure_values_ns=")
            .append(plan[0]).append(',').append(plan[1]).append(',')
            .append(plan[2]).append(',').append(plan[3]).append(',')
            .append(plan[4]).append('\n');
        report.append("plan_validation=PASS\n");
    ...
    private boolean consumeUiLaunchArm() {
        File arm = privateFile(UI_LAUNCH_ARM_NAME);
        try {
            if (!arm.isFile()) return false;
            String value = readFirstLine(arm);
            if (!arm.delete()) return false;
            return UI_LAUNCH_ARM_VALUE.equals(value);
        } catch (Throwable ignored) {
            deleteQuietly(arm);
            return false;
        }
    ...
    private static long[] readAndValidatePlan(File file) throws Exception {
        if (!file.isFile() || file.length() < 1L || file.length() > 160L) {
            throw new IllegalStateException("missing_or_oversized_adaptive_plan");
        }
        String text = readAscii(file);
        String canonical = text.endsWith("\n")
            ? text.substring(0, text.length() - 1) : text;
        if (canonical.indexOf('\n') >= 0 || canonical.indexOf('\r') >= 0) {
            throw new IllegalStateException("adaptive_plan_not_one_line");
        }
        String[] fields = canonical.split(" ", -1);
        if (fields.length != 6 || !PLAN_VERSION.equals(fields[0])) {
            throw new IllegalStateException("adaptive_plan_wrong_shape_or_version");
        }
        long[] values = new long[5];
        for (int i = 0; i < values.length; i++) {
            if (!fields[i + 1].matches("[0-9]+")) {
                throw new IllegalStateException("adaptive_plan_non_decimal");
            }
            values[i] = Long.parseLong(fields[i + 1]);
            if (values[i] < 10000L || values[i] > 20000000L) {
                throw new IllegalStateException("adaptive_plan_exposure_out_of_range");
            }
        }
        double spanEv = Math.log(values[4] / (double) values[0])
            / Math.log(2.0);
        if (values[1] != values[0] || values[2] < values[0]
                || values[3] < values[2] || values[4] < values[3]
                || values[4] <= values[0] || spanEv < 0.5) {
            throw new IllegalStateException("adaptive_plan_roles_or_order_invalid");
        }
        String rebuilt = PLAN_VERSION + " " + values[0] + " " + values[1]
            + " " + values[2] + " " + values[3] + " " + values[4];
        if (!rebuilt.equals(canonical) || !text.equals(rebuilt + "\n")) {
            throw new IllegalStateException("adaptive_plan_not_canonical");
        }
        return values;
    }
    ...
    private static boolean validPassResult(String text) {
        return "L16_HOSTLESS_A_GROUP_ADAPTIVE_INLINE_AF_CAPTURE_V1".equals(
