...
# Build the clean-room ARM32 preload prototype without an Android sysroot.
set -eu

if [ "$#" -ne 1 ]; then
    printf 'usage: %s /absolute/output/liblcc_async_writer_shim.so\n' "$0" >&2
    exit 2
fi
...
"$CLANG" \
    ...
    -Wl,--build-id=sha1 -Wl,-soname,liblcc_async_writer_shim.so \
    -o "$OUTPUT"
