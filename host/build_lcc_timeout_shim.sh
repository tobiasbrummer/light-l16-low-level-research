...
# Build the clean-room ARM32 capture-timeout preload without an Android sysroot.
set -eu

if [ "$#" -ne 1 ]; then
    printf 'usage: %s /absolute/output/liblcc_open_camera_timeout_shim.so\n' "$0" >&2
    exit 2
fi

SCRIPT_DIR=$(CDPATH= cd -- "$(dirname -- "$0")" && pwd)
REPO_ROOT=$(dirname "$SCRIPT_DIR")
SOURCE="$REPO_ROOT/shim/lcc_open_camera_timeout_shim.c"
OUTPUT=$1
...
"$CLANG" \
    ...
    -Wl,--build-id=sha1 -Wl,-soname,liblcc_open_camera_timeout_shim.so \
    -o "$OUTPUT"
