...
# Build the fixed A1 center-focus/capture gate for 32-bit LightOS.
set -eu

if [ "$#" -ne 1 ]; then
    printf 'usage: %s /absolute/output/liblcc_a1_focus_capture_shim.so\n' "$0" >&2
    exit 2
fi

SCRIPT_DIR=$(CDPATH= cd -- "$(dirname -- "$0")" && pwd)
REPO_ROOT=$(dirname "$SCRIPT_DIR")
SOURCE="$REPO_ROOT/shim/lcc_a1_focus_capture_shim.c"
OUTPUT=$1
...
"$CLANG" \
    ...
    -Wl,--build-id=sha1 -Wl,-soname,liblcc_a1_focus_capture_shim.so \
    -o "$OUTPUT"
